...
$('saveTranscription').onclick=async()=>{ const provider=document.querySelector('input[name="provider"]:checked').value, recognitionEngine=$('recognitionEngine').value, model=document.querySelector('input[name="localModel"]:checked')?.value; const state=await api.transcription.get(); if(provider==='synapse'&&recognitionEngine==='parakeet')return alert('Parakeet runs locally. Select Local · Apple Silicon.'); if(provider==='local'&&recognitionEngine==='whisper'&&!state.models[model]?.installed)return alert('Select an installed Whisper model first.'); if(provider==='local'&&recognitionEngine==='parakeet'&&!state.parakeet.installed)return alert('Select a folder containing a Parakeet ONNX model and tokens.txt first.'); await api.transcription.save({provider,recognitionEngine,model,language:$('language').value,modelsPath:$('modelsPath').value.trim()}); $('saveTranscription').textContent='Settings saved'; };
