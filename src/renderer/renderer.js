...
$('saveTranscription').onclick=async()=>{ const engine=document.querySelector('input[name="engine"]:checked').value, model=document.querySelector('input[name="localModel"]:checked').value; const state=await api.transcription.get(); if(engine==='local'&&!state.models[model]?.installed)return alert('Select an installed local model first.'); await api.transcription.save({engine,model,language:$('language').value,modelsPath:$('modelsPath').value.trim()}); $('saveTranscription').textContent='Settings saved'; };
