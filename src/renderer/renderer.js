...
async function refreshTranscription() { const state=await api.transcription.get(), prefs=state.preferences; $('modelsPath').value=prefs.modelsPath; $('recognitionEngine').value=prefs.recognitionEngine; document.querySelector(`input[name="provider"][value="${prefs.provider}"]`).checked=true; await renderProvider(prefs.provider,prefs); const whisper=prefs.recognitionEngine==='whisper'; $('localModels').hidden=prefs.provider==='synapse'||!whisper; $('parakeetStatus').hidden=prefs.provider==='synapse'||whisper; $('parakeetStatus').className=`connection-status ${state.parakeet.installed?'ok':'bad'}`; $('parakeetStatus').textContent=state.parakeet.installed?`Parakeet ready · ${state.parakeet.model}`:'Parakeet model not found. Select a folder containing a NeMo Parakeet ONNX model and tokens.txt.'; $('localModels').innerHTML=Object.entries(state.models).map(([id,m])=>`<label class="model-option"><input type="radio" name="localModel" value="${escapeHtml(id)}" ${prefs.model===id?'checked':''}><span><strong>${escapeHtml(m.label)}</strong><span>${escapeHtml(m.detail)}${m.installed?` · ${escapeHtml(m.path)}`:''}</span></span>${m.installed?'<b>Ready</b>':`<button type="button" data-download="${id}">Download</button>`}</label>`).join(''); if(!document.querySelector('input[name="localModel"]:checked'))document.querySelector('input[name="localModel"]')?.click(); document.querySelectorAll('[data-download]').forEach(button=>button.onclick=async event=>{ event.preventDefault(); button.disabled=true; button.textContent='Downloading…'; try{ await api.transcription.download(button.dataset.download); await refreshTranscription(); }catch(e){ alert(e.message); button.disabled=false; button.textContent='Download'; } }); }
async function renderProvider(provider,prefs={}) { const cloud=provider==='synapse'; $('synapseModels').hidden=!cloud; $('modelsPath').closest('label').hidden=cloud; $('recognitionEngine').closest('label').hidden=cloud; if(!cloud)return; $('synapseModels').innerHTML='<p>Loading models from Synapse…</p>'; try{ const models=await api.config.models(); $('synapseModels').innerHTML=models.map(model=>`<label class="model-option"><input type="radio" name="synapseModel" value="${escapeHtml(model.id)}" ${prefs.synapseModel===model.id||(!prefs.synapseModel&&model.id==='whisper-1@v2-large')?'checked':''}><span><strong>${escapeHtml(model.id)}</strong><span>${escapeHtml(model.mode||'audio transcription')}</span></span></label>`).join('')||'<p>No transcription models returned by Synapse.</p>'; }catch(e){ $('synapseModels').innerHTML=`<p>${escapeHtml(e.message)}</p>`; } }
